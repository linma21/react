...
/*
  ...
  JSX 
    - Javascript XML
    - 브라우저에 출력할 HTML 태그를 동적 생성하기 위한 Javascipt XML 문법
...
function App() {
  ...
  const cities = ["서울", "대전", "대구", "광주", "부산"];
  const users = [
    { uid: "a101", name: "김유신", age: 23, addr: "김해" },
    { uid: "a102", name: "김춘추", age: 34, addr: "경주" },
    { uid: "a103", name: "장보고", age: 54, addr: "완도" },
    { uid: "a104", name: "강감찬", age: 32, addr: "서울" },
    { uid: "a105", name: "이순신", age: 19, addr: "부산" },
  ];
  ...
  const onSubmitHandler = (e) =>{
      ...
      const formData = {
        ...
        name: e.target.uid.name,
        age: e.target.uid.age
      };
      console.log("폼 전송" + formData)
  };
  // return 아래부터 JSX
  return (
    <div className="App">
      ...
      {/*
        ...
        - JSX는 Class 속성 대산 className 속성 사용
      */}
      <ul>
      ...
      */}
      ...
      <h4>이벤트</h4>
      {/* prettier-ignore */}
      <button onClick={()=>{ alert('!!!');}}>버튼1</button>
      <button onClick={onClickHandler}>버튼2</button>

      <p>
        count : <span className="count">0</span><br></br>
        {/* JSX는 반드시 종료태그 선언 */}
